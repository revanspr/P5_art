...
function draw() {
    ...
    // Save frames for 10 seconds (300 frames at 30fps)
    if (recording && frameNumber < recordingDuration * fps) {
        // Pad frame number with zeros (e.g., 0001, 0002, etc.)
        let paddedFrame = nf(frameNumber, 4);
        saveCanvas(`frame_${paddedFrame}`, 'png');
        frameNumber++;
        console.log(`Saved frame ${frameNumber} of ${recordingDuration * fps}`);
    } else if (frameNumber >= recordingDuration * fps) {
        recording = false;
        console.log('Recording complete! All 300 frames saved.');
